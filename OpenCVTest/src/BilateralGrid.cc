...
void BilateralGrid::construct_AMatrix_step2_for_depth()
{
	...
	std::cout << "temp.dims = " << mat_depth.dims << "temp.size = [";
	for(int i = 0; i < mat_depth.dims; ++i) {
		if(i) std::cout << " X ";
		std::cout << mat_depth.size[i];
	}
	std::cout << "] temp.channels = " << mat_depth.channels() << std::endl;
...
void BilateralGrid::execute_ICCG(int iter, float eps)
{
	...
	#endif
	cvtColor(mat_colorized, mat_colorized, COLOR_YCrCb2BGR);
}
